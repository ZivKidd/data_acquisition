...
NUM_IMAGES = 1000000000000  # number of images to grab
# folder=''
...
def configure_trigger(cam):
    ...
    try:
        ...
        node_trigger_selector_ExposureActive = node_trigger_selector.GetEntryByName('ExposureActive')
        if not PySpin.IsAvailable(node_trigger_selector_ExposureActive) or not PySpin.IsReadable(
                node_trigger_selector_ExposureActive):
            print('Unable to set trigger selector (enum entry retrieval). Aborting...')
            return False
        node_trigger_selector.SetIntValue(node_trigger_selector_ExposureActive.GetValue())
        print('Trigger selector set to Exposure Active...')
...
def acquire_images(cam, nodemap, nodemap_tldevice,pub):
    """
        ...
        for i in range(NUM_IMAGES):
            ...
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    #     # filename =r'E:\calib\data\0830pg\\'\
                    #     #           + device_serial_number+'-'+str(i).zfill(6)+'-'+str(time.time())+'.bmp'
                    #     # filename =r'./image_trigger//'\
                    #     #           + device_serial_number+'-'+str(i).zfill(8)+'-'+str(time.time())+'.bmp'
                    #     filename =os.path.join(folder,
                    #                            device_serial_number+'-'+str(i).zfill(8)+'-'+str(rospy.Time.now())+'.bmp')
                    #
                    ...
                    image.data=image_data.tostring()
                    image.header=header
                    pub.publish(image)
                    # # Save image
                    # #
...
def run_single_camera(cam,pub):
    ...
    try:
        ...
        # Acquire images
        result &= acquire_images(cam, nodemap, nodemap_tldevice,pub)
...
def main():
    # if not os.path.exists('./image_trigger'+str(rospy.Time.now())+'//'):
    # folder = str(rospy.Time.now())

    # os.mkdir(folder)

    rospy.init_node('flir_node',anonymous=True)

    pub0 = rospy.Publisher('flir_cam0', Image, queue_size=10)
    pub1 = rospy.Publisher('flir_cam1', Image, queue_size=10)
    """
    ...
    #     p = Process(target=run_single_camera, args=cam)  # 实例化进程对象
    #     p.start()
    ...
    #     p.join()
    # cam=cam_list[0]
    # p0 = Process(target &=run_single_camera, args=(cam_list[0]))
    # p1 = Process(target &=run_single_camera, args=(cam_list[1]))
    # p0.start()
    # p1.start()
    # p0.join()
    # p1.join()

    t1 = threading.Thread(target=run_single_camera, args=(cam_list[0],pub0))
    t2 = threading.Thread(target=run_single_camera, args=(cam_list[1],pub1))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
